...
-- 2021-02-25,SSparks,Created File
-- 2021-02-26,SSparks,Continued Building Views and Functions
-- 2021-03-02,SSparks,Added alias to number 8
-- 2021-03-03,SSparks,Completed and finalized for submission
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_StevenSparks')
	 Begin 
	  Alter Database [Assignment07DB_StevenSparks] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_StevenSparks;
	 End
	Create Database Assignment07DB_StevenSparks;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_StevenSparks;
...
--With SCHEMABINDING (Dave, you said last time we don't need this on these views, right?)
As
Select Top 1000000
  ProductName
  ,[InventoryDate] =  
...
-- Check that it works: Select * From vProductInventories; -- DAVE, the select statements were already commented out and I didn't
                                                           -- see any directions telling us to move them, so I didn't.
go
...
Create or Alter Function fProductInventoriesWithPreviousMonthCountsWithKPIs(@CountVsPreviousCountKPI int)
Returns Table
As
    Return 
	  (Select Top 1000000
